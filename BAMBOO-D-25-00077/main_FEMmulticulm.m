...
for jj=1:length(alphav)
    ...
    for hh=1:np-1
        ...
        [Kstinv]=PrismaticBeam(E,Ii(np-hh),dz);
        ind=(hh-1)*2+1:2*hh+2;
        Kg(ind,ind)=Kg(ind,ind)+nc/2*Kst+nc/2*Kstinv;
        Mi=zeros(4,4);
        Mi(1,1)=nc/2*rho*Ai(hh)*dz/2+nc/2*rho*Ai(np-hh)*dz/2;
        Mi(3,3)=nc/2*rho*Ai(hh)*dz/2+nc/2*rho*Ai(np-hh)*dz/2;

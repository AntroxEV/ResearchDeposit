...
@author: at924
"""
def modelWind(xdamp,target,caseW):
    ...
    op.numberer('Plain')			    # renumber dof's to minimize band-width (optimization), if you want to
    op.system('BandGeneral')		    # how to store and solve the system of equations in the analysis
    op.test('NormDispIncr', 1.0e-4,  5, 5 )
    op.algorithm('Newton')                 # use Linear algorithm for linear analysis
    ...
    np.savetxt('uTOP' + caseW + '_OPT2.txt',uTOP)
    np.savetxt('aTOP'+ caseW + '_OPT2.txt',aTOP)
    np.savetxt('time'+ caseW + '_OPT2.txt',time)
    np.savetxt('MYmud'+ caseW + '_OPT2.txt',MYmud)
    np.savetxt('OPTdamp'+ caseW + '_OPT2.txt',xdamp)

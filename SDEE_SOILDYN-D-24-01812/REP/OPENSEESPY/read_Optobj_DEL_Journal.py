...
# plt.figure(3)
# plt.plot(xs3,eta3,'ob',label='Values for fitting')
# xs2=range(0,51)
...
plt.plot(xs[:-1], line, '--r', label=r' $\beta={:.2f}w_s +{:.2f}$, $R^2 =$ {:.2f} '.format(slope,intercept,r_value))
hfont = {'fontname':'Arial'}
plt.xlabel('Mean (total) velocity at the reference height [m/s]',**hfont, fontsize=12)
plt.ylabel(r'Stiffness proportional coefficient $\beta$  [-]',**hfont, fontsize=12)
plt.ylim([0,0.6])

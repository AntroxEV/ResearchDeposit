...
#INPUT FILES
caseW='_20ms'
FX=np.genfromtxt('FX'  + caseW + '.txt')
FZ=np.genfromtxt('FZ' + caseW + '.txt')
MY=np.genfromtxt('MY' + caseW + '.txt')
timeF=list(FX[:,0])
windFX=list(FX[:,1])
windFZ=list(FZ[:,1])
windMY=list(MY[:,1])
...
timeSeries('Path', 2,   '-dt', 0.005, '-values', *windFX,'-prependZero', '-factor',0.001)
timeSeries('Path', 3,  '-dt', 0.005, '-values', *windFZ,'-prependZero', '-factor',0.001)
timeSeries('Path', 4,  '-dt', 0.005, '-values', *windMY,'-prependZero', '-factor',0.001)
...
while tCurrent <= 120:
    ...
    MYmud.append(eleForce(15, 3))
    Amud.append(eleForce(15, 1))
    TP.append(nodeDisp(12,3))

    
print(np.max(np.abs(TP)))
print(np.max(np.abs(Amud)))
#plt.plot(uTOP)

...
@author: at924
"""
def modelWind(xdamp,target,caseW,ntask):
    ...
    for i in range(0,90):
        op.uniaxialMaterial('Elastic', 1+i, Es[i], np.multiply(eta1,Es[i]))
        op.uniaxialMaterial('Elastic', 91+i, Ers[i], np.multiply(eta2,Ers[i]))
    ...
    np.savetxt('time_SET'+str(ntask)+ caseW + '_OPT.txt',time)

...
def modelWind(xdamp,target,tmax,caseW,ntask):
    ...
    while tCurrent <= tmax:
        ok = op.analyze(1, .01)
        #print('time',tCurrent,'ntask',ntask)
        if ok != 0:
            print('noncovergence')
        tCurrent = op.getTime()
    ...
    #np.savetxt('uTOP_SET'+str(ntask) + caseW + '_OPT.txt',uTOP)
    #np.savetxt('aTOP_SET'+str(ntask)+ caseW + '_OPT.txt',aTOP)
    #np.savetxt('time_SET'+str(ntask)+ caseW + '_OPT.txt',Atime)
    #np.savetxt('MYmud_SET'+str(ntask)+ caseW + '_OPT.txt',MYmud)
    np.savetxt('uTOP_SET'+str(ntask) + caseW + '.txt',uTOP)
    np.savetxt('aTOP_SET'+str(ntask)+ caseW + '.txt',aTOP)
    np.savetxt('time_SET'+str(ntask)+ caseW + '.txt',Atime)
    np.savetxt('MYmud_SET'+str(ntask)+ caseW + '.txt',MYmud)
    optim=np.divide(np.abs(np.max(np.abs(MYmud[12000:]))-target),target)
    print('optim',optim,'SET',ntask)
    #elapsed_time = time.process_time() - startRun
    #print('elapsed time (minutes):',np.divide(elapsed_time,60))
    return optim

...
def modelWind(xdamp,target,tmax,caseW):
    ...
    np.savetxt('uTOP' + caseW +'_'+str(eta)+ '_DEL.txt',uTOP)
    np.savetxt('aTOP'+ caseW +'_'+str(eta)+ '_DEL.txt',aTOP)
    np.savetxt('time'+ caseW +'_'+str(eta)+ '_DEL.txt',Atime)
    np.savetxt('MYmud'+ caseW +'_'+str(eta)+ '_DEL.txt',MYmud)
    ...
    DEL=np.power(np.divide(np.sum(np.power(np.multiply(array_out[0],array_out[3]),mcoef)),Nref),1/mcoef)
    print('DEL',DEL)
    optim=np.divide((DEL-target),target)
    print('optim',optim)

...
@author: at924
"""
def modelWind(xdamp,target,tmax,caseW,ntask):
    ...
    FX=np.genfromtxt('./Random/'+caseW+'/SET'+str(ntask) + '/FX'  + caseW + '.txt')
    FZ=np.genfromtxt('./Random/'+caseW+'/SET'+str(ntask) + '/FZ'  + caseW + '.txt')
    MY=np.genfromtxt('./Random/'+caseW+'/SET'+str(ntask) + '/MY'  + caseW + '.txt')
    timeF=list(FX[:,0])
    windFX=list(FX[:,1])
    windFZ=list(FZ[:,1])
    windMY=list(MY[:,1])
    ...
    #np.savetxt('time'+ caseW + '_OPT.txt',time)
    #np.savetxt('MYmud'+ caseW + '_OPT.txt',MYmud)
    np.savetxt('uTOP_SET'+str(ntask) + caseW + '_DEL.txt',uTOP)
    np.savetxt('aTOP_SET'+str(ntask)+ caseW + '_DEL.txt',aTOP)
    np.savetxt('time_SET'+str(ntask)+ caseW + '_DEL.txt',Atime)
    np.savetxt('MYmud_SET'+str(ntask)+ caseW + '_DEL.txt',MYmud)
    
    Nref=2E8;
    mcoef=4;
    
    #array_out = rf.rainflow(np.array(MYmud[12000:]))
    #array_out = array_out[:,array_out[0,:].argsort()]
    #DEL=np.power(np.divide(np.sum(np.multiply(np.power(array_out[0],mcoef),array_out[3])),Nref),1/mcoef)
    rngT=[]
    countT=[]
    for rng, a , count, b,c in rf.extract_cycles(MYmud[12000:]):
        rngT.append(rng)
        countT.append(count)
    DEL = np.power(np.divide(np.sum(np.multiply(np.power(rngT,mcoef),countT)),Nref),1/mcoef)
    optim=np.abs(np.divide((DEL-target),target))
    print('optim',optim,'DEL',DEL,'target',target)
